...
Это канонический документ по проду. `deploy/README.md` (ручной сервер) и
`deploy/APP_PLATFORM.md` (Timeweb App Platform) — исторические/альтернативные;
по факту прод работает по этому файлу. App Platform **не использовался**: его
сборщик оказался нестабильным с монорепо (баги контекста сборки, клонирования и
режимов), поэтому развёрнуто на обычном VPS со стандартным `docker compose`.
