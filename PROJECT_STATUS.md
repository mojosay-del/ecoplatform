...
Дата последнего обновления: 2026-05-30.

## Текущий этап

**🚀 MVP ЗАДЕПЛОЕН В ПРОД (2026-05-30).** Сайт работает: **https://ecoplatform.pro**
(Timeweb VPS `81.200.158.7` + `docker compose`: caddy/web/api/redis; внешние
Managed PostgreSQL и S3, HTTPS через Caddy/Let's Encrypt). Как всё развёрнуто и
**как выкатывать обновления без поломки БД** — в
[deploy/PRODUCTION.md](deploy/PRODUCTION.md). Timeweb App Platform пробовали и
отказались (нестабильный сборщик с монорепо: баги контекста сборки, клонирования,
режимов) — развёрнуто вручную на обычном сервере, где стандартный `docker compose`
...
Волна 12 — CMS-полишинг и админ-таблицы — закрыта полностью. Полный
технический codebase-аудит MVP завершён; дальнейший этап — ручная приёмка MVP
владельцем продукта.
...
Внеплановая локальная DB-консолидация закрыта: данные старого Docker-проекта
`ecoplatform_v10crm` на PostgreSQL 16 перенесены через dump/restore в текущий
PostgreSQL 18 (`ecoplatform-postgres-1`, порт `5433`). После проверки
счётчиков CMS, `FileAsset`, S3-обложек и `/api/ready` старые контейнер,
volume и сеть `ecoplatform_v10crm` удалены; в локальном Docker должны
оставаться контейнеры текущего проекта `ecoplatform-postgres-1` и
`ecoplatform-redis-1`.
...
Волна 11.6 закрыта: регистрация стала двухшаговой — сначала компания и тип, затем личные данные, доступ и согласия; кнопка «Назад» сохраняет данные обоих шагов. ИНН не запрашивается при регистрации: пользователь заполняет реквизиты позже в профиле компании.
...
Дата: 2026-05-28 (после завершения полного технического codebase-аудита).
