...
- Pre-MVP-модули (форум, магазин решений) заведены в enum'ах и сайдбаре, но без
  ...
  миграции с реальными платежами на проде. Торговая площадка управляется
  настройкой `marketplace.enabled`: на время закрытого тестирования она скрыта
  из меню и закрыта на API, а платные действия при включении проверяются в сервисах.
