...
Машиночитаемый OpenAPI-контракт доступен в dev/test на `/api/openapi.json`,
Swagger UI — на `/api/docs`. В production эти маршруты выключены по умолчанию;
для внешней публикации API-документации нужно явно задать `OPENAPI_ENABLED=1`.
...
apps/
  api/                         NestJS-сервер
    prisma/
      ...
      migrations/              60+ SQL-миграций PostgreSQL (с 2026-05-20)
...
- «Магазин решений» пока заведён в enum'ах и сайдбаре, но без UI — это сделано
  осознанно как «фундамент на вырост», чтобы не двигать позже миграции с
  реальными платежами на проде. (Форум и торговая площадка уже реализованы
  полностью — раздел не про них.)
- Торговая площадка реализована, но управляется настройкой `marketplace.enabled`:
  на время закрытого тестирования она скрыта из меню и закрыта на API, а платные
  действия при включении проверяются в сервисах.
- Платёжный провайдер (Тинькофф-Касса), верификация телефона по SMS и per-seat
  биллинг (`Company.seatsPurchased`) отложены до релиза — модели и enum'ы стоят,
  но код оплаты/списания мест ещё не подключён.
