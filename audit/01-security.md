...
### 4. Stored-XSS через MIME-тип в загруженных файлах ✅ DONE 2026-05-25
> После ревизии перед Волной 6 закрыто фактически: `files.service.ts` определяет реальный MIME по magic-number (`file-type/fromBuffer`), блокирует HTML/SVG/executable-типы и опасные расширения, сверяет declared MIME с detected MIME, а не-media файлы получают `Content-Disposition: attachment` в S3. Для cover-загрузки `processCoverImage()` получает уже проверенный MIME и перекодирует изображение в WebP. Unit-тесты покрывают HTML-as-image, SVG и PDF attachment.
