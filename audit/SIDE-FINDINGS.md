...
### S-8. CMS-редакторы дают горизонтальный overflow на мобильной ширине

- **Что**: во время проверки 12.4 на ширине 390px `/admin/content/news` и `/admin/content/knowledge-base` расширяют страницу из-за существующих элементов редактора блоков (`block-row`, `rich-text-toolbar`, save-bar), хотя новые breadcrumbs уже не создают overflow.
- **Что сделать**: вернуться в рамках CMS-задач 12.6–12.8, когда редактор блоков всё равно будет дорабатываться; отдельно проверить mobile 390px для news/education/knowledge-base редакторов.
