...
Реализация 2026-05-27: auto-save подключён к редакторам новостей, уроков и базы знаний через общий `useCmsAutosave`. Срабатывает для существующих черновиков каждые 30 секунд и при уходе фокуса из формы; опубликованные записи сохраняются вручную, чтобы не публиковать незавершённые правки без отдельной draft-ревизии.
...
Реализация 2026-05-27: `/api/admin/dashboard` отдаёт KPI, 30-дневный ряд регистраций и последние audit-события только для роли `admin`; `/admin` показывает KPI-карточки, анимированный график регистраций, ленту последних событий и сохранённый центр быстрых переходов. `content_manager` и `moderator` остаются на своих быстрых ссылках без доступа к KPI.

## Проверки Волны 12

- [ ] /admin/companies — таблица, сортировка работает.
- [ ] /admin/journals — таблица, видны diff'ы из Волны 9.7.
- [x] /admin — дашборд с KPI.
- [x] CMS auto-save срабатывает.
