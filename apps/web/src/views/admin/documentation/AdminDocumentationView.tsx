...
// выбранной записи. Близнец AdminKnowledgeView, но документ — первоклассная
// сущность: файл, версия, «действует с», закрепление и отметка обновления.
      ...
      <section className="page">
        ...
        <div className="moderation-layout cms-vertical-layout">
          <div className="education-tree">
            ...
            {categoryCreateOpen ? (
              <DocCategoryCreateForm onCreate={createCategory} onClose={() => setCategoryCreateOpen(false)} />
            ) : null}
            ...
            <ul className="tree" role="tree">
              {categories.map((category) => (
                <DocCategoryNode
                  ...
                  onSelect={startEdit}
                  onAddDocument={() => startNewDocument(category.id)}
                  onReorder={(event) => void reorderDocuments(category.id, event)}
                />
              ))}
