...
import { Clock, Coins, Plus, Route, TrendingDown, TrendingUp, Truck, Weight } from "lucide-react";
...
const WORKER_COLORS = ["var(--info)", "var(--brand)", "var(--logo-green)", "var(--warning-strong)", "var(--material-makulatura)"];
...
export function RetailCalculatorView() {
  ...
  const matPrice = settings.materialPrices.find((price) => price.slug === material) ?? { slug: material, buy: "0", sell: "0" };
  ...
  }, [matPrice.buy, matPrice.sell, veh, settings.fuelPrice, settings.loadTime, settings.otherCosts, settings.workers, distance, weight]);
  ...
  const addVehicle = () => {
    ...
    update((prev) => ({
      ...
      vehicles: [...prev.vehicles, { id, name: `Машина ${prev.vehicles.length + 1}`, fuel: "15", deprec: "8", speed: "50" }],
    }));
  };
    ...
    <AppShell>
      <section className="page tc-page">
        <div className="tc-shell">
          ...
          <div className="tc-vehicle-chips">
            {settings.vehicles.map((vehicle) => (
              <button
                key={vehicle.id}
                type="button"
                className={`tc-vehicle-chip${vehicle.id === vehicleId ? " is-active" : ""}`}
          ...
          <section className="tc-card">
            ...
            <div className="tc-fields">
              ...
              <Field label="Цена закупки" unit="₽/кг" value={matPrice.buy} onChange={(value) => setMatPrice("buy", value)} />
              <Field label="Цена продажи" unit="₽/кг" value={matPrice.sell} onChange={(value) => setMatPrice("sell", value)} />
            </div>
